...
def retrieved_question_links(question, decay=0.5):
    ...
    for i, tag in enumerate(tags):
        ...
        results = link_retriever.vectorstore.similarity_search_with_score(
            tag, k=5)
...
# links from the most recent retrieval (used as "already_visited")
retrieved_links = []
...
h3_tags = [h3.get_text(strip=True).replace("\u00b6", "").strip()
...
def chat(req: ChatRequest):
    with closing(sqlite3.connect(DB_PATH)) as conn:
        row = conn.execute(
            "SELECT id FROM sessions WHERE id = ?", (req.session_id,)).fetchone()
    if not row:
